...
$("#word_text, #definition").click(function() {
	...
	$.get("<?php echo $view['router']->generate('jameswuvocabbundle.main.word.def')?>" + '/' + current_word, function(data){
		console.log(data);
...
$(".btn").click(function() {
	$.post("<?php echo $view['router']->generate('jameswuvocabbundle.main.word.status')?>", { word_id: $('#word_text').attr('word_id'), status: $(this).attr('status') });
	$.get("<?php echo $view['router']->generate('jameswuvocabbundle.main.word.random')?>", function(data){
		console.log(data);
